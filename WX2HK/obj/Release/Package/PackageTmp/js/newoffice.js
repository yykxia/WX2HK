...
function TANGER_OCX_OpenDoc(url,userid,fileid,type)
{   
	 TANGER_OCX_OBJ = document.all.item("TANGER_OCX");
	 var filespec1,filespec2,ft_char;
	 ft_char=type;
	 filespec1="office/"+fileid+type;
	 filespec2="/laoa/modefile/"+url+type
	 var xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
     xmlhttp.open("GET",filespec1,false)
     xmlhttp.send()
	 if(404 == xmlhttp.status)
       {
	   xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
	   xmlhttp.open("GET",filespec2,false)
       xmlhttp.send()
       if(404 == xmlhttp.status)
		{newdoc(type,userid);} 
	   else
		{try{TANGER_OCX_OBJ.OpenFromurl(filespec2);}catch(err){};}
	   }
	 else
	   try{TANGER_OCX_OBJ.OpenFromurl(filespec1);}catch(err){};
}

function newdoc(type,USERID)
{   
    if (type=='.doc')
...
function TANGER_OCX_SaveEditToServerDisk(filename,fileid)
{
	alert(filename);
	...
	try
	{
		...
		"uploadedit.aspx",  
		"EDITFILE",	
		"action=newoffice&fileid="+fileid, //other params seperrated by '&'. For example:myname=tanger&hisname=tom
		TANGER_OCX_filename //filename
...
{   
     document.all.item("modefile").disabled=true;
	 var filespec
     filespec="modefile/"+document.all.item("modefile").value;
     var xmlhttp = new ActiveXObject("Microsoft.XMLHTTP");
     xmlhttp.open("GET",filespec,false)
     xmlhttp.send()
     if(404 == xmlhttp.status)
	 {
        alert("该文件不存在或者文件模板类型与附件类型不对应!");
     }
	 else
	    TANGER_OCX_OBJ.OpenFromURL(filespec);	
}
