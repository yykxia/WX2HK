...
public class BarCodeHelper
{
    ...
    public static Image MakeBarcodeImage_WH(string InputData, int BarWeight, bool AddQuietZone,int ImageHeight)
    {
        // get the Code128 codes to represent the message
        Code128Content content = new Code128Content(InputData);
        int[] codes = content.Codes;
        ...
        //draw the codetext bottom
        height += ImageHeight;//调整条码的整体显示宽度
        // get surface to draw on
        Image myimg = new System.Drawing.Bitmap(width, height);
        using (Graphics gr = Graphics.FromImage(myimg))
        {
            ...
            for (int codeidx = 0; codeidx < codes.Length; codeidx++)
            {
                ...
                for (int bar = 0; bar < 8; bar += 2)
                {
                    ...
                    if (barwidth > 0)
                    {
                        gr.FillRectangle(System.Drawing.Brushes.Black, cursor, 5, barwidth, height - 100);
                    }
            ...
            gr.DrawString(InputData, new Font("Arial", 80), Brushes.Black, new PointF(50, height - 100));
        }
        return myimg;
